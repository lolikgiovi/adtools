...
export function initQuickQuery(container, updateHeaderTitle) {
  ...
  function handleSimulationFillSchema() {
    ...
    const sampleData = [
      ...
      ["SUBMISSION_DATE", "TIMESTAMP", "Yes", "", "4"],
  ...
  const DATE_FORMATS = {
    DATE_ONLY: {
      ...
      regex:
        /^(\d{2}[-/\.]\d{2}[-/\.]\d{4}|\d{4}[-/\.]\d{2}[-/\.]\d{2}|\d{1,2}\/\d{1,2}\/\d{4}|\d{2}-[A-Z]{3}-\d{2,4}|\d{8})$/i,
    ...
    DATE_TIME: {
      ...
      regex:
        /^(\d{2}[-/\.]\d{2}[-/\.]\d{4}|\d{4}[-/\.]\d{2}[-/\.]\d{2}|\d{2}-[A-Z]{3}-\d{4}|\d{8})\s\d{2}:\d{2}(:\d{2})?$/i,
    ...
    TIMESTAMP: {
      ...
      regex:
        /^(\d{2}[-/\.]\d{2}[-/\.]\d{4}|\d{4}[-/\.]\d{2}[-/\.]\d{2}|\d{8})\s\d{2}[:.]\d{2}[:.]\d{2}[,\.]\d{1,9}$/,
    ...
    TIMESTAMP_AM_PM: {
      ...
      regex:
        /^\d{1,2}[-/\.]\d{1,2}[-/\.]\d{4}\s\d{1,2}:\d{2}(:\d{2})?(\.\d{1,6})?\s[AP]M$/i,
